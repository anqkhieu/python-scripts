...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if (num_rolls == 0):
        turn_points = free_bacon(opponent_score)
    elif (num_rolls > 0):
        turn_points = roll_dice(num_rolls, dice)
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while (score0 < goal) and (score1 < goal):
        if (player == 0):
            dicerolls = strategy0(score0, score1)
            if (dicerolls == 0):
                score0 += free_bacon(score1)
            else:
                score0 += take_turn(dicerolls, score1, dice)
        if (player == 1):
            dicerolls = strategy1(score1, score0)
            if (dicerolls == 0):
                score1 += free_bacon(score0)
            else:
                score1 += take_turn(dicerolls, score0, dice)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        if (who == 0):
            current_score_comparison = score0
        if (who == 1):
            current_score_comparison = score1
        new_high = previous_high
        difference = current_score_comparison - previous_score
        ...
        return announce_highest(who, new_high, current_score_comparison)
    return say
    # END PROBLEM 7
...
def make_averaged(fn, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def functionaverage(*args):
        result = 0
        counter = 0
        while(num_samples > counter):
            result += fn(*args)
            counter = counter + 1
